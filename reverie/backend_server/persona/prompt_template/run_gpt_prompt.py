...
##############################################################################
# CHAPTER 1: Run Bedrock Prompt
##############################################################################

def run_bedrock_prompt_wake_up_hour(persona, test_input=None, verbose=False): 
  """
  ...
  bedrock_param = {"max_tokens": 5, 
             "temperature": 0.8, "top_p": 1, "stop": ["\n"]}
...
def run_bedrock_prompt_daily_plan(persona, 
                              wake_up_hour, 
                              test_input=None, 
                              verbose=False):
  """
...
def run_bedrock_prompt_generate_hourly_schedule(persona, 
                                            curr_hour_str,
                                            p_f_ds_hourly_org, 
                                            hour_str,
                                            intermission2=None,
  ...
  bedrock_param = {"max_tokens": 50, "temperature": 0.5, "top_p": 1, "stop": ["\n"]}
...
def run_bedrock_prompt_task_decomp(persona, 
                               task, 
                               duration, 
                               test_input=None, 
                               verbose=False): 
  ...
  def __func_clean_up(bedrock_response, prompt=""):
    ...
    # temp = [i.strip() for i in bedrock_response.split("\n")]
    # _cr = []
    # cr = []
    ...
    try:
        # 응답을 줄 단위로 분리
        lines = [line.strip() for line in bedrock_response.split('\n') if line.strip()]
        cr = []
        ...
        for line in lines:
            ...
            try:
                ...
                duration_str = line.split('(')[1].split(',')[0].strip()
  ...
  def __func_validate(bedrock_response, prompt=""): 
    ...
    try: 
      __func_clean_up(bedrock_response)
    except: 
      pass
      # return False
  ...
  print ("?????")
  print (prompt)
  output = safe_generate_response(prompt, bedrock_param, 5, get_fail_safe(),
                                   __func_validate, __func_clean_up)
  ...
  brain/get_next_action_v3.py", line 364, in run_bedrock_prompt_task_decomp
...
def run_bedrock_prompt_action_sector(action_description, 
                                persona, 
                                maze, 
                                test_input=None, 
                                verbose=False):
...
def run_bedrock_prompt_action_arena(action_description, 
                                persona, 
...
def run_bedrock_prompt_action_game_object(action_description, 
                                      persona, 
                                      maze,
                                      temp_address,
                                      test_input=None, 
...
def run_bedrock_prompt_pronunciatio(action_description, persona, verbose=False): 
  def create_prompt_input(action_description): 
    if "(" in action_description: 
  ...
  def __func_validate(bedrock_response, prompt=""): 
    try: 
      __func_clean_up(bedrock_response, prompt="")
      if len(bedrock_response) == 0: 
        return False
    except: return False
    return True 
...
def run_bedrock_prompt_event_triple(action_description, persona, verbose=False): 
  ...
  bedrock_param = {"max_tokens": 30, "temperature": 0, "top_p": 1, "stop": ["\n"]}
...
def run_bedrock_prompt_act_obj_desc(act_game_object, act_desp, persona, verbose=False): 
...
def run_bedrock_prompt_act_obj_event_triple(act_game_object, act_obj_desc, persona, verbose=False): 
...
def run_bedrock_prompt_new_decomp_schedule(persona, 
                                       main_act_dur, 
                                       truncated_act_dur, 
                                       start_time_hour,
                                       end_time_hour, 
  ...
  def __func_clean_up(bedrock_response, prompt=""):
    new_schedule = prompt + " " + bedrock_response.strip()
  ...
  def __func_validate(bedrock_response, prompt=""): 
    try: 
      bedrock_response = __func_clean_up(bedrock_response, prompt)
      dur_sum = 0
      for act, dur in bedrock_response: 
        dur_sum += dur
...
def run_bedrock_prompt_decide_to_talk(persona, target_persona, retrieved,test_input=None, 
                                       verbose=False): 
  ...
  def __func_validate(bedrock_response, prompt=""): 
    try: 
      if bedrock_response.split("Answer in yes or no:")[-1].strip().lower() in ["yes", "no"]: 
        return True
      return False     
    except:
      return False 
  ...
  def __func_clean_up(bedrock_response, prompt=""):
    return bedrock_response.split("Answer in yes or no:")[-1].strip().lower()
...
def run_bedrock_prompt_decide_to_react(persona, target_persona, retrieved,test_input=None, 
                                       verbose=False): 
  def create_prompt_input(init_persona, target_persona, retrieved, 
                          test_input=None): 
  ...
  def __func_validate(bedrock_response, prompt=""): 
    try: 
      if bedrock_response.split("Answer: Option")[-1].strip().lower() in ["3", "2", "1"]: 
        return True
      return False     
    except:
      return False 
  ...
  def __func_clean_up(bedrock_response, prompt=""):
    return bedrock_response.split("Answer: Option")[-1].strip().lower() 
...
def run_bedrock_prompt_create_conversation(persona, target_persona, curr_loc,
  ...
  def __func_clean_up(bedrock_response, prompt=""):
    ...
    bedrock_response = (prompt + bedrock_response).split("What would they talk about now?")[-1].strip()
    content = re.findall('"([^"]*)"', bedrock_response)

    speaker_order = []
    for i in bedrock_response.split("\n"): 
...
def run_bedrock_prompt_summarize_conversation(persona, conversation, test_input=None, verbose=False): 
  ...
  def __func_clean_up(bedrock_response, prompt=""):
    ret = "conversing about " + bedrock_response.strip()
    return ret
...
def run_bedrock_prompt_extract_keywords(persona, description, test_input=None, verbose=False): 
  ...
  def __func_clean_up(bedrock_response, prompt=""):
    print ("???")
    print (bedrock_response)
    bedrock_response = bedrock_response.strip().split("Emotive keywords:")
    factual = [i.strip() for i in bedrock_response[0].split(",")]
    emotive = [i.strip() for i in bedrock_response[1].split(",")]
...
def run_bedrock_prompt_keyword_to_thoughts(persona, keyword, concept_summary, test_input=None, verbose=False): 
...
def run_bedrock_prompt_convo_to_thoughts(persona, 
                                    init_persona_name,  
                                    target_persona_name,
                                    convo_str,
...
def run_bedrock_prompt_event_poignancy(persona, event_description, test_input=None, verbose=False): 
  ...
  def __func_clean_up(bedrock_response, prompt=""):
    bedrock_response = int(bedrock_response.strip())
    return bedrock_response
  ...
  # Bedrock Plugin ===========================================================
  def __chat_func_clean_up(bedrock_response, prompt=""): ############
    bedrock_response = int(bedrock_response)
    return bedrock_response
...
def run_bedrock_prompt_thought_poignancy(persona, event_description, test_input=None, verbose=False): 
  def create_prompt_input(persona, event_description, test_input=None): 
    prompt_input = [persona.scratch.name,
                    persona.scratch.get_str_iss(),
                    persona.scratch.name,
...
def run_bedrock_prompt_chat_poignancy(persona, event_description, test_input=None, verbose=False): 
  def create_prompt_input(persona, event_description, test_input=None): 
    prompt_input = [persona.scratch.name,
                    persona.scratch.get_str_iss(),
                    persona.scratch.name,
...
def run_bedrock_prompt_focal_pt(persona, statements, n, test_input=None, verbose=False): 
  def create_prompt_input(persona, statements, n, test_input=None): 
    prompt_input = [statements, str(n)]
    return prompt_input
  
  def __func_clean_up(bedrock_response, prompt=""):
    bedrock_response = "1) " + bedrock_response.strip()
    ret = []
    for i in bedrock_response.split("\n"): 
      ret += [i.split(") ")[-1]]
    return ret
  ...
  # Bedrock Plugin ===========================================================
  def __chat_func_clean_up(bedrock_response, prompt=""): ############
    ret = ast.literal_eval(bedrock_response)
    return ret
...
def run_bedrock_prompt_insight_and_guidance(persona, statements, n, test_input=None, verbose=False): 
  def create_prompt_input(persona, statements, n, test_input=None): 
    prompt_input = [statements, str(n)]
    return prompt_input
...
def run_bedrock_prompt_agent_chat_summarize_ideas(persona, target_persona, statements, curr_context, test_input=None, verbose=False): 
...
def run_bedrock_prompt_agent_chat_summarize_relationship(persona, target_persona, statements, test_input=None, verbose=False): 
...
def run_bedrock_prompt_agent_chat(maze, persona, target_persona,
                               curr_context, 
                               init_summ_idea, 
  ...
  def __func_clean_up(bedrock_response, prompt=""):
    ...
    bedrock_response = (prompt + bedrock_response).split("Here is their conversation.")[-1].strip()
    content = re.findall('"([^"]*)"', bedrock_response)
...
def run_bedrock_prompt_summarize_ideas(persona, statements, question, test_input=None, verbose=False): 
...
def run_bedrock_prompt_generate_next_convo_line(persona, interlocutor_desc, prev_convo, retrieved_summary, test_input=None, verbose=False): 
...
def run_bedrock_prompt_generate_whisper_inner_thought(persona, whisper, test_input=None, verbose=False): 
...
def run_bedrock_prompt_planning_thought_on_convo(persona, all_utt, test_input=None, verbose=False): 
...
def run_bedrock_prompt_memo_on_convo(persona, all_utt, test_input=None, verbose=False): 
  def create_prompt_input(persona, all_utt, test_input=None): 
    prompt_input = [all_utt, persona.scratch.name, persona.scratch.name, persona.scratch.name]
    return prompt_input
...
def run_bedrock_generate_safety_score(persona, comment, test_input=None, verbose=False): 
  ...
  def __chat_func_validate(bedrock_response, prompt=""): 
    try: 
      fields = ["output"]
      response = json.loads(bedrock_response)
      for field in fields: 
...
def run_bedrock_generate_iterative_chat_utt(maze, init_persona, target_persona, retrieved, curr_context, curr_chat, test_input=None, verbose=False): 
  ...
  def __chat_func_clean_up(bedrock_response, prompt=""): 
    bedrock_response = extract_first_json_dict(bedrock_response)

    cleaned_dict = dict()
    cleaned = []
    for key, val in bedrock_response.items(): 
      cleaned += [val]
  ...
  def __chat_func_validate(bedrock_response, prompt=""): 
    ...
    try: 
      ...
      print (extract_first_json_dict(bedrock_response))
      # print ("debug 3")
